...
public class AppTimerManager {
    ...
    private static final String KEY_SUSPENDED_TODAY_PREFIX = "suspended_today_";
    ...
    public static void setAppLimitMinutes(Context context, String packageName, int limitMinutes) {
        ...
        if (limitMinutes <= 0) {
            prefs.edit().remove(KEY_TIMER_PREFIX + packageName)
                        .remove(KEY_SUSPENDED_TODAY_PREFIX + packageName).apply();
        } else {
    ...
    public static boolean isSuspendedToday(Context context, String packageName) {
        return getPrefs(context).getBoolean(KEY_SUSPENDED_TODAY_PREFIX + packageName, false);
    }

    public static void setSuspendedToday(Context context, String packageName, boolean suspended) {
        getPrefs(context).edit().putBoolean(KEY_SUSPENDED_TODAY_PREFIX + packageName, suspended).apply();
    }
    ...
    public static void checkSinglePackageLimit(Context context, String packageName) {
        ...
        try {
            ...
            boolean wasSuspendedToday = isSuspendedToday(context, packageName);

            Log.i(TAG, "Single package check for " + packageName + ": used " + (usedMillis / 60000) + "m / limit " + limitMins + "m, suspendedToday=" + wasSuspendedToday);

            if (usedMillis >= limitMillis || wasSuspendedToday) {
                setSuspendedToday(context, packageName, true);
                dpm.setPackagesSuspended(DeviceAdminReceiver.getComponentName(context), new String[]{packageName}, true);
                Log.i(TAG, "EXCEEDED OR REINSTALLED LIMIT: Instantly suspended " + packageName);
            } else {
                dpm.setPackagesSuspended(DeviceAdminReceiver.getComponentName(context), new String[]{packageName}, false);
            }
        } catch (Exception e) {
            Log.e(TAG, "Error in checkSinglePackageLimit for " + packageName, e);
        }
    }
    ...
    public static void checkAndEnforceLimits(Context context) {
        try {
            ...
            for (Map.Entry<String, Integer> entry : limits.entrySet()) {
                checkSinglePackageLimit(context, entry.getKey());
            }
        } catch (Exception e) {
    ...
    public static void performMidnightReset(Context context) {
        try {
            Log.i(TAG, "Executing Midnight Reset: clearing suspended_today flags...");
            SharedPreferences prefs = getPrefs(context);
            SharedPreferences.Editor editor = prefs.edit();
            for (String key : prefs.getAll().keySet()) {
                if (key.startsWith(KEY_SUSPENDED_TODAY_PREFIX)) {
                    editor.remove(key);
                }
            }
            editor.apply();
            registerAllObservers(context);
        } catch (Exception e) {
            Log.e(TAG, "Error in performMidnightReset", e);
        }
    }
