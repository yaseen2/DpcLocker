...
import com.afwsamples.testdpc.AppTimerManager;
import com.afwsamples.testdpc.BrowserBlocker;
...
public class PolicyManagementFragment extends BaseSearchablePolicyPreferenceFragment implements
    ...
    private static final String AUTO_BLOCK_BROWSERS_KEY = "auto_block_browsers";
    private static final String APP_USAGE_TIMERS_KEY = "app_usage_timers";
    ...
    public void onCreatePreferences(Bundle savedInstanceState, String rootKey) {
        ...
        mAutoBlockBrowsersPreference = (DpcSwitchPreference) findPreference(AUTO_BLOCK_BROWSERS_KEY);
        if (mAutoBlockBrowsersPreference != null) {
            mAutoBlockBrowsersPreference.setChecked(BrowserBlocker.isAutoBlockEnabled(getActivity()));
            mAutoBlockBrowsersPreference.setOnPreferenceChangeListener(this);
        }
    ...
    private void showAppUsageTimersDialog() {
        final Context context = getActivity();
        if (context == null) return;

        final PackageManager pm = context.getPackageManager();
        List<ApplicationInfo> installedApps = pm.getInstalledApplications(0);
        final List<ApplicationInfo> userApps = new java.util.ArrayList<>();
        final List<String> appLabels = new java.util.ArrayList<>();

        for (ApplicationInfo app : installedApps) {
            if (pm.getLaunchIntentForPackage(app.packageName) != null && !context.getPackageName().equals(app.packageName)) {
                userApps.add(app);
        ...
        builder.setTitle("App Daily Usage Timers");
        builder.setItems(appLabels.toArray(new String[0]), new DialogInterface.OnClickListener() {
            @Override
            public void onClick(DialogInterface dialog, int which) {
                final ApplicationInfo selectedApp = userApps.get(which);
                final String appName = pm.getApplicationLabel(selectedApp).toString();
                int currentLimit = AppTimerManager.getAppLimitMinutes(context, selectedApp.packageName);

                final EditText input = new EditText(context);
                input.setInputType(InputType.TYPE_CLASS_NUMBER);
                input.setText(currentLimit > 0 ? String.valueOf(currentLimit) : "30");

                new AlertDialog.Builder(context)
                        .setTitle("Set Limit for " + appName)
                        .setMessage("Enter daily limit in minutes (0 for Unlimited):")
                        .setView(input)
                        .setPositiveButton("Save", new DialogInterface.OnClickListener() {
                            ...
                            public void onClick(DialogInterface d, int w) {
                                ...
                                AppTimerManager.setAppLimitMinutes(context, selectedApp.packageName, mins);
                                Toast.makeText(context, "Timer updated for " + appName, Toast.LENGTH_SHORT).show();
                            }
                        })
