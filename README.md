...
* **YouTube Shorts Lockdown (`URLBlocklist` & Unhook Force-Install):**
  * Enforces `*://*.youtube.com/shorts/*` in Chrome & Edge `URLBlocklist`.
  * **`ExtensionInstallForcelist`**: Force-installs the official **Unhook** extension (`khncbfacgiphjaedlcngbpeoehbdplbd`) in Chrome & Edge. The extension cannot be disabled or uninstalled, and automatically strips YouTube Shorts from home feeds, sidebars, and Single Page App (SPA) `pushState` navigations!
* **Chrome & Edge Direct Connection Lockdown:**
  ...
  * **100% Custom Extension Freedom:** Installing, developing, and using custom Chrome/Edge extensions is **100% allowed and unrestricted**.
...
├── enable_windows_protection.ps1       # Windows PowerShell Script (SafeSearch, Cloudflare Family DNS, Notorious Domain Block, Proxy Direct & Unhook Force-Install)
...
### 1. Windows Setup (Adult Content, SafeSearch, Cloudflare Family DNS, Unhook Shorts Lock & VPN Lock)
...
* **Cloudflare Family DNS:** Sets system DNS to `1.1.1.3` and `1.0.0.3` (blocks adult domains system-wide without hiding YouTube comments).
...
* **Unhook Shorts Lock (`ExtensionInstallForcelist`):** Force-installs Unhook extension to strip Shorts from YouTube Single Page App (SPA) navigations.
...
* **Full Extension Freedom:** Custom extension development and Web Store installs remain 100% allowed.
...
* **Chrome & Edge Registry Policies:**
  ...
  * `ForceYouTubeRestrict` = `0` *(Disables YouTube Restricted Mode so comments work)*
  * `SafeSitesFilterBehavior` = `1` *(Enforces Chrome adult site filter)*
  * `DnsOverHttpsMode` = `"off"` *(Disables Secure DNS DoH bypass)*
  * `URLBlocklist` = `["*fboxtv.org*", "*x.com*", "*twitter.com*", "*twimg.com*", "*redd.it*", "*tumblr.com*", "*telegram.org*", "*t.me*", "*croxyproxy.com*", "*proxysite.com*", "*hide.me*", "*blockaway.net*", "*://*.youtube.com/shorts/*"]`
...
3. **`ForceYouTubeRestrict` = `0`**: Disables YouTube Restricted Mode on Android Chrome so YouTube comments load normally.
4. **`ProxyMode` = `"direct"`**: Forces direct connections in Android Chrome, preventing proxy extension overrides.
5. **`URLBlocklist`**: `["fboxtv.org", "x.com", "twitter.com", "twimg.com", "reddit.com", "redditmedia.com", "redd.it", "tumblr.com", "telegram.org", "t.me", "croxyproxy.com", "proxysite.com", "hide.me", "blockaway.net", "*://*.youtube.com/shorts/*"]`.
